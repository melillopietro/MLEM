...
with tab5:
    ...
    mitre_db_file = "mitre_definitions.json"; mitre_data = {}
    if os.path.exists(mitre_db_file):
        with open(mitre_db_file, "r") as f: mitre_data = json.load(f)
        st.success(f"Database Online ({len(mitre_data)} techniques)")
    else: st.warning("Database missing. Run 'update_mitre_db.py'.")
    
    q = st.text_input("🔍 Search TTP (e.g. T1566):").upper().strip()
    if q:
        if q in mitre_data:
            st.subheader(f"{q}: {mitre_data[q]['name']}")
            st.info(mitre_data[q]['description'])
            st.markdown(f"[View on MITRE](https://attack.mitre.org/techniques/{q.replace('.','/')})")
